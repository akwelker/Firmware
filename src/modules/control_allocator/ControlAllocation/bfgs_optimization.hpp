    ...
    bfgs_settings_t* settings_inp)
{
    ...
    const float grad_err_tol = 1E-08f;
        ...
        -> float
    {
        ...
        double eps_flt2 = EPSILON_DOUBLE;
        float ret;
        ...
        if (grad_out2) {
            ...
            for (size_t i = 0; i < n_vals; ++i) {
                jacob_matrix(i,i) = std::exp((double) vals_inp2(i)) * (2* eps_flt2 + (double) upper_bounds(i) - (double) lower_bounds(i)) \
                                / (std::exp(2 * (double) vals_inp2(i)) + 2*std::exp((double) vals_inp2(i)) + 1);
            }
